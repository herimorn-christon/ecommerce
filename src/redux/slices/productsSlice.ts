    ...
    searchProducts: (state, action: PayloadAction<string>) => {
      ...
      if (searchTerm === '') {
        state.filteredProducts = state.selectedCategory 
          ? state.products.filter(product => product.categoryId === state.selectedCategory?.id)
          : state.products;
      } else {
        ...
        state.filteredProducts = state.selectedCategory 
          ? filtered.filter(product => product.categoryId === state.selectedCategory?.id)
          : filtered;
      }
    }
  },
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchProducts.fulfilled, (state, action: PayloadAction<Product[]>) => {
        ...
        state.filteredProducts = state.selectedCategory 
          ? action.payload.filter(product => product.categoryId === state.selectedCategory?.id) 
          : action.payload;
      })
...
export const { setSelectedCategory, clearSelectedProduct, searchProducts } = productsSlice.actions;
